...
def call(def stage) {
  ...
  sh "rsync -ra -e ssh --include '*/' --include='*.log' --include='*.txt' --include='*.conf' --include='*.xml' --include='*.properties' --include='*.html' --include='*.png' --exclude='*' --prune-empty-dirs * ${env.CIX_WLOGS_HOST}:${remoteDir}"
