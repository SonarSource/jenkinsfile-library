class burgr implements Serializable {
    def fakeCommit(url, branch, commit, timestamp) {
        println 'Simulate a notification to BURGR from GitHub (push, PRs, ...)'
    ...
    def notifyBurgr(url, branch, commit, step, type, status, timestamp) {
        println "Send a step notification to BURGR: [repo: ${url}, branch: ${branch}, commit: ${commit}, step: ${step}, type: ${type}, status: ${status}]"
    ...
    def Map extractDataForBurgrFromURL(url,timestamp) {
